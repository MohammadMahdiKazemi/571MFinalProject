...
from distloss import DistillationLoss
import torchvision
logger = logging.get_logger(__name__)


def train_epoch(
    train_loader, model,teacher_model, optimizer, train_meter, cur_epoch, cfg, writer=None
):
    """
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        criterion = DistillationLoss(loss_fun, teacher_model, 0.5)
        ...
        #efloss = loss_fun(preds.squeeze()[:,0], labels.squeeze()[:,0])
        #esedloss = loss_fun(preds.squeeze()[:,1:3], labels.squeeze()[:,1:3])
        # loss = efloss
        #loss = efloss + esedloss/2.5
        #print("preds", preds.squeeze())
        #print("labels", labels.squeeze())
        loss, base_loss = criterion(inputs, preds, labels)
        ...
        if cfg.DETECTION.ENABLE:
            if cfg.NUM_GPUS > 1:
                ...
                base_loss = du.all_reduce([base_loss])[0]
            loss = loss.item()
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                if cfg.NUM_GPUS > 1:
                    ...
                    [base_loss] = du.all_reduce([base_loss])
                loss = loss.item()
                base_loss = base_loss.item()
                # efloss = efloss.item()
            else:
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                loss = loss_fun(preds.squeeze(), labels.squeeze())
                efloss = loss #loss_fun(preds.squeeze()[:,0], labels.squeeze()[:,0])
                if cfg.NUM_GPUS > 1:
                    loss, top1_err, top5_err = du.all_reduce(
                        [loss, efloss, loss]
                    )
...
# # Distillation
#     teacher_model = None
#     teacher_model_name="r2plus1d_18"
#     teacher_weights="/scratch/st-puranga-1/users/MohammadMahdi/Workspace/timesformerM/dist_check/best.pt"#"/AS_Neda/timeSformer/pretrained/checkpoint_paper.pt"
#     device = None
#     if device is None:
#         device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

#     # Set up model
#     teacher_model = torchvision.models.video.__dict__[teacher_model_name](pretrained=pretrained)

#     teacher_model.fc = torch.nn.Linear(teacher_model.fc.in_features, 1)
...
#         checkpoint = torch.load(teacher_weights)
#         model.load_state_dict(checkpoint['state_dict'])
#         print("weights resnet2+1D loaded")
        
#     teacher_model.eval()
#     #Distillation
    # teacher_model = build_model(cfg, teacher_flag=True)
...
def train(cfg):
    ...
    # # Set up model
    # teacher_model = torchvision.models.video.__dict__["r2plus1d_18"](pretrained=False)
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        # Train for one epoch.
        train_epoch(
            train_loader, model,teacher_model, optimizer, train_meter, cur_epoch, cfg, writer
        )
